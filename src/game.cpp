...
void _updateAndDraw(GameState& gs) 
{
    ...
    if (delta < 0.1f) {
        ...
        if (gs.net.steamOk) {
            if (IsKeyPressed(KEY_L) && !gs.net.inLobby && !gs.net.lobbyPending) {
                _netCreateLobby(gs);
            }
            if (IsKeyPressed(KEY_J) && !gs.net.inLobby && !gs.net.lobbyPending) {
                _netJoinLobbyFromClipboard(gs);
            }
            if (IsKeyPressed(KEY_C) && gs.net.inLobby) {
                _netCopyLobbyIdToClipboard(gs);
            }
        }
    }
...
void _drawUI(GameState& gs) {
    ...
    if (gs.net.steamOk) {
        ...
        std::string lobbyText = "Lobby: none (L=create, J=join)";
        if (gs.net.lobbyPending) {
            lobbyText = "Lobby: connecting...";
        } else if (gs.net.inLobby) {
            lobbyText = "Lobby: " + std::to_string(gs.net.lobbyId.ConvertToUint64()) + " (C=copy)";
        }
        DrawTextEx(gs.ga.p->font, lobbyText.c_str(), {10.0f, 42.0f}, 13, 1, WHITE);

        std::string peersText = "Peers: " + std::to_string(gs.net.peerCount) + "/" + std::to_string(MAX_PLAYERS - 1);
        DrawTextEx(gs.ga.p->font, peersText.c_str(), {10.0f, 58.0f}, 13, 1, WHITE);
    } else {
